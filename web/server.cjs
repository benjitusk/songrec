...
// Proxy all /api/shazam/* requests to Shazam's API
app.all('/api/shazam/*', (req, res) => {
  // Extract the path after /api/shazam/
  const shazamPath = req.path.replace('/api/shazam', '')
  const query = req.url.includes('?') ? req.url.slice(req.url.indexOf('?')) : ''
  const targetUrl = `https://amp.shazam.com${shazamPath}${query}`
